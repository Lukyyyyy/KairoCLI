...
浏览并添加项目。只有管理员能浏览服务器允许根目录并向用户授权目录；普通用户只能看到
自己已获授权的项目。目录浏览默认限制在启动工作区的父目录内，只返回文件夹信息并忽略
符号链接；若要访问另一个目录树，请在目标目录或其同级目录中启动 Web 服务。
...
扫码后选择管理员已授权的工作区并启用通道，微信消息会进入 SQLite 持久 inbox，再由
