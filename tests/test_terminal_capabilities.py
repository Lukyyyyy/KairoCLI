...
def test_inline_requires_an_interactive_non_dumb_terminal() -> None:
    ...
    assert resolve_renderer_mode(
        "inline", input_is_tty=False, output_is_tty=True
    ).mode == "plain"
...
def test_tui_requires_real_minimum_terminal_and_honors_no_tui() -> None:
    ...
    assert resolve_renderer_mode(
        "tui", columns=120, rows=40, no_tui=True, **normal
    ).mode == "inline"
    assert resolve_renderer_mode(
        "tui", input_is_tty=False, output_is_tty=False
    ).mode == "plain"


def test_plain_and_no_color_semantics_do_not_require_ansi() -> None:
    assert resolve_renderer_mode(
        "plain", input_is_tty=False, output_is_tty=False, term="dumb"
    ).mode == "plain"
